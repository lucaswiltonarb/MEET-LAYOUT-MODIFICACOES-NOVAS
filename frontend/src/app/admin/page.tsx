  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white">
      <form onSubmit={login} data-testid="admin-login-form" className="w-full max-w-sm p-8 rounded-2xl bg-slate-800 shadow-2xl border border-slate-700">
        ...
        <p className="text-slate-400 text-sm mb-6">Moogle Meet · SimulaMarketing</p>
        ...
        <input
          ...
          autoFocus
          className="w-full rounded-lg bg-slate-900 px-4 py-3 border border-slate-700 focus:border-blue-500 outline-none mb-4"
        />
        {error && <div className="text-red-400 text-sm mb-3" data-testid="admin-login-error">{error}</div>}
        <button
          data-testid="admin-login-submit"
          disabled={loading || !password}
          className="w-full py-3 rounded-lg bg-blue-600 hover:bg-blue-700 disabled:opacity-50 font-medium"
        >
