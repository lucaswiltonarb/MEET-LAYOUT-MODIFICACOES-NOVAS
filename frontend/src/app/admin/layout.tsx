...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (pathname === '/admin') { setAuthed(true); return; }
  ...
  return (
    <div className="min-h-screen flex bg-slate-950 text-slate-100">
      <aside className="w-64 bg-slate-900 border-r border-slate-800 p-6 flex flex-col">
        <div className="mb-8">
          <div className="text-2xl font-light">SimulaMarketing</div>
          <div className="text-xs text-slate-500 uppercase tracking-wider mt-1">Admin Panel</div>
        </div>
        ...
        <button
          ...
          onClick={logout}
          className="px-4 py-2.5 rounded-lg text-sm text-slate-300 hover:bg-slate-800 text-left"
        >
          Sair
        </button>
      </aside>
